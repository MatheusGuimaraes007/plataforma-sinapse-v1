...
<template>
  ...
  <aside
    ...
    <nav class="flex-1 p-4 space-y-1">
      ...
      <router-link
        to="/costumerNumberConnection"
        class="block px-3 py-1 rounded transition bg-snpLightBlue/30"
        :class="isActive('/contatos') ? 'bg-white/15' : 'hover:bg-snpLightBlue/50'"
      >
        <span class="l text-snpDarkBlue">Conexões WhatsApp</span>
      </router-link>
    </nav>
    <div class="p-4 gap-1 flex">
      <router-link 
      to="/"
      class="w-[40%] px-3 py-2 rounded bg-snpDarkBlue text-white font-medium hover:bg-snpDarkBlue/90 cursor-pointer transition flex justify-center">
        <span>Sair</span>
      </router-link>
    </div>
  </aside>
